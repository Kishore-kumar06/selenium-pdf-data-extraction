...
def extract_pipelinename_metadata(pdf):
    ...
    full_pipeline_pattern = re.compile(rf"^.*(?:Pipeline)?.*{suffix_pattern}\.?$", re.IGNORECASE)
    continuation_pattern = re.compile(rf"^(?:PIPELINE,\s+)?{suffix_pattern}\.?$", re.IGNORECASE)
...
def extract_effectivedate_metadata(pdf):
    page1_text = pdf.pages[0].extract_text() or ""
    match_effective = re.search(
        r"\bEffective(?:\s+Date)?\b\s*:?\s*([A-Za-z]+\s+\d{1,2},\s+\d{4})",
        page1_text,
        re.IGNORECASE,
    )
    if not match_effective:
        return ""

    dt_obj = datetime.strptime(match_effective.group(1), "%B %d, %Y")
...
def normalize_header_cell(value):
    value = clean(value).replace("\r", " ").replace("\n", " ")
    value = value.replace(":", "")
...
def is_origin_header(value):
    ...
    return (
        ...
        or value.startswith("receipt points/origin")
        or value.startswith("receipt points origin")
    )
...
def is_rate_header(value):
    ...
    return (
        ...
        or value.startswith("uncommitted")
...
def find_expected_header(table):
    ...
    for row_idx in range(rows_to_check):
        ...
        first_cell = normalize_header_cell(row[0]) if row else ""
        ...
        for col_idx in scan_indices:
            cell = row[col_idx]
            if is_origin_header(cell) and origin_idx is None:
                origin_idx = col_idx
            elif is_destination_header(cell) and destination_idx is None:
...
def split_rate_cell(rate_text):
    ...
    if bracketed:
        return [clean(m) for m in bracketed]
    na_matches = re.findall(r"N/?A", rate_text, re.IGNORECASE)
    if na_matches:
        return [clean(m).upper() for m in na_matches]
    return [rate_text] if is_rate(rate_text) else []


def is_rate(value):
    return bool(NUMERIC_RATE_RE.match(clean(value)))
...
def strip_page_number_tail(value):
    return re.sub(r"\s+\d+$", "", clean(value))
...
def split_multiple_origins(text):
    ...
    for part in parts:
        ...
        paren_chunks = [clean(m) for m in re.findall(r"[^()]+?\([^()]+\)", part)]
...
def _parse_single_rate_section_from_words(page, header_idx, next_header_idx=None):
    ...
    for line in lines[header_idx + 1:end_idx]:
        ...
        origin_text = clean(" ".join(w["text"] for w in line["words"] if w["x0"] < split_origin_dest))
        destination_text = clean(" ".join(w["text"] for w in line["words"] if split_origin_dest <= w["x0"] < split_dest_rate))
        rate_text = clean(" ".join(w["text"] for w in line["words"] if w["x0"] >= split_dest_rate))

        suffix_match = RATE_SUFFIX_RE.match(rate_text)
        if suffix_match and is_rate(suffix_match.group(2)):
            prefix = clean(suffix_match.group(1))
            if prefix:
                destination_text = clean(f"{destination_text} {prefix}")
            rate_text = clean(suffix_match.group(2))
        ...
        if is_rate(rate_text):
            ...
            current_row = {
                "origin": clean(" ".join(pending_origin_parts + ([origin_text] if origin_text else []))),
                "destination": clean(" ".join(pending_destination_parts + ([destination_text] if destination_text else []))),
                "rate": rate_text,
            }
            pending_origin_parts = []
            pending_destination_parts = []
            ...
            if origin_text and destination_text:
                if current_row is not None:
                    current_row["origin"] = clean(f"{current_row['origin']} {origin_text}")
                    current_row["destination"] = clean(f"{current_row['destination']} {destination_text}")
                else:
...
# ------------------------------------------------------------
def extract_midway_style_from_text(page_text, pipeline_name="", effective_date="", page_num=None):
    records = []
    if not page_text:
        return records
    ...
    block = clean(block_match.group(1))
    ...
    origin_destination_text = clean(origin_destination_text)
    ...
    if not dest_match:
        return records
    destination = clean(dest_match.group(1))
    origin_text = clean(origin_destination_text.replace(dest_match.group(1), ""))
            ...
            records.append(
                {
                    ...
                    "LiquidRateCentsPerBbl": clean(rate),
                }
            )
    return records


def extract_cheyenne_from_words(page, pipeline_name="", effective_date="", page_num=None):
    records = []
    words = page.extract_words(use_text_flow=False, keep_blank_chars=False)
    lines = _cluster_words_to_lines(words)
    ...
    for line in lines:
        ...
        if rate_matches:
            rates.extend(clean(r) for r in rate_matches)
        ...
        if "Wyoming" in text and text.count("Wyoming") >= 2 and "Platte Station" in text and "Cheyenne" in text:
            match = re.match(r"(Platte Station, Guernsey, Wyoming)\s+(Cheyenne, Wyoming)", clean(text))
            if match:
...
def extract_diamond_from_words(page, pipeline_name="", effective_date="", page_num=None):
    records = []
    words = page.extract_words(use_text_flow=False, keep_blank_chars=False)
    lines = _cluster_words_to_lines(words)
    ...
    for line in lines:
        ...
        origin_text = clean(" ".join(w["text"] for w in line["words"] if w["x0"] < split_origin_dest))
        dest_text = clean(" ".join(w["text"] for w in line["words"] if split_origin_dest <= w["x0"] < split_dest_base))
        base_text = clean(" ".join(w["text"] for w in line["words"] if split_dest_base <= w["x0"] < split_base_contract))
        contract_text = clean(" ".join(w["text"] for w in line["words"] if w["x0"] >= split_base_contract))
    ...
    destination = clean(" ".join([p for p in dest_parts if p not in {"Memphis,"} or True]))
    if destination:
...
def extract_generic_word_sections(page, pipeline_name="", effective_date="", page_num=None, split_origins=False):
    records = []
    words = page.extract_words(use_text_flow=False, keep_blank_chars=False)
    if not words:
        return records
...
def extract_midway_from_words(page, pipeline_name="", effective_date="", page_num=None):
    records = []
    ...
    for line in block_lines:
        ...
        cleaned_line = clean(cleaned_line)
        if rate_tokens:
    ...
    destination = clean(" ".join(destination_parts))
    origins = split_multiple_origins(clean(" ".join(origin_parts)))
...
def extract_from_tables(page, pipeline_name, effective_date, page_num, last_header_info=None):
    unpivoted_data = []
    ...
    for table in tables:
            ...
            for row in cleaned_table[data_start_row:]:
                ...
                origin = clean(row[origin_idx]) if len(row) > origin_idx else ""
                destination = clean(row[destination_idx]) if len(row) > destination_idx else ""
                raw_rates = [clean(row[idx]) if len(row) > idx else "" for idx in rate_indices]
...
# ------------------------------------------------------------
def extract_data(pdf, source_name=""):
    unpivoted_data = []
    ...
    for page_num, page in enumerate(pdf.pages, start=1):
        ...
        page_records, last_header_info = extract_from_tables(
            page, pipeline_name, effective_date, page_num, last_header_info
        )

        if not page_records:
            if "midway pipeline" in source_lower or ("Viscosity Fee" in text and "CONTRACT RATE" in text and "BASE RATE" in text):
                page_records = extract_midway_from_words(page, pipeline_name, effective_date, page_num) or extract_midway_style_from_text(text, pipeline_name, effective_date, page_num)
            elif "cheyenne pipeline" in source_lower:
                page_records = extract_cheyenne_from_words(page, pipeline_name, effective_date, page_num)
            elif "diamond pipeline" in source_lower:
                page_records = extract_diamond_from_words(page, pipeline_name, effective_date, page_num)
            elif "panola pipeline" in source_lower:
                page_records = extract_generic_word_sections(page, pipeline_name, effective_date, page_num, split_origins=True)
            elif "texas express pipeline" in source_lower:
                page_records = extract_generic_word_sections(page, pipeline_name, effective_date, page_num, split_origins=False)
        ...
        unpivoted_data.extend(page_records)
    ...
    for file in get_transformed_files(curr_path):
        ...
        with pdfplumber.open(file) as pdf:
            ...
            if final_data is not None and len(final_data) > 0:
                output_file = os.path.join('extracted_data', input_file)
